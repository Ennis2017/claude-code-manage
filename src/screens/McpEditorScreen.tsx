...
import { readTextFileMeta, writeJsonFile, detectExternalChange, revealInFinder, createFile } from '../lib/fs-bridge';
...
export function McpEditorScreen(props: Props) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const meta = await readTextFileMeta(filePath);
        if (!mounted) return;
        const content = meta.exists ? meta.content : '{\n  "mcpServers": {}\n}\n';
        ...
        toast_msg(`读取 .mcp.json 失败：${String(e)}`, 'error');
        setLoaded(true);
      }
    })();
  ...
  const doSave = async (force = false) => {
    ...
    try {
      if (!force && exists) {
        const changed = await detectExternalChange(filePath, mtime).catch(() => false);
        if (changed) {
          toast_msg('磁盘已被外部修改，请刷新后重试', 'error');
          setSaving(false);
          return;
        }
      }
      const result = exists
        ? await writeJsonFile(filePath, draft, force ? null : mtime)
        : await createFile(filePath, draft);
      setBaseline(draft);
      setMtime(result.mtime);
      setSize(result.size_bytes);
      setExists(true);
      setEditing(false);
      toast_msg('已保存 · .mcp.json', 'success');
      scanAll();
    } catch (e) {
        ...
        <div style={{ flex: 1, overflow: 'auto', padding: '20px 28px 24px' }}>
          <div style={{ marginBottom: 16 }}>
            ...
            <div style={{ fontSize: 11.5, color: 'var(--cc-muted)', display: 'flex', gap: 14, flexWrap: 'wrap' }}>
              ...
              <span>· {(size / 1024).toFixed(1)} KB · 修改于 {mtime || '—'}</span>
