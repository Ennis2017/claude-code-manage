...
/** route.screen / route.cmd → InnerEntry（项目侧多了 'local'） */
...
export function ProjectDetail() {
  ...
  // route.screen 变化（侧边 Rail 切换项目、Dashboard 跳转等）→ 同步当前 active tab
  useEffect(() => {
    if (route.name !== 'project' || !p) return;
    if (!route.screen) return;
    const next: TabContent = { kind: 'inner', inner: routeToInnerEntry(route.screen, route.cmd) };
    setActiveTabContent(p.id, next);
  }, [route.name, route.screen, route.cmd, p?.id, setActiveTabContent]);
  ...
  const tabTitleForInner = (e: InnerEntry): string => {
    ...
    if (e.entry === 'mcp') return '.mcp.json';
  ...
  return (
    <>
      <TabbedConfigShell
        scopeId={SCOPE}
        rail={<Rail active="projects" projectId={p.id} />}
        ...
        onPlus={() => {
          ...
          if (route.screen !== 'overview') {
            go({ name: 'project', id: p.id, screen: 'overview' });
          }
        }}
      />
      {confirmRemove && (
...
type ProjectSnap = NonNullable<ReturnType<typeof useConfigStore.getState>['snapshot']>['projects'][number];

interface ProjectInnerViewProps {
  inner: InnerEntry;
  project: ProjectSnap;
  setNewKind: (k: EntryKind | null) => void;
  onConfirmRemove: () => void;
  toast_msg: (msg: string, tone?: 'info' | 'success' | 'error') => void;
  go: ReturnType<typeof useAppStore.getState>['go'];
}

function ProjectInnerView({ inner, project: p, setNewKind, onConfirmRemove, toast_msg, go }: ProjectInnerViewProps) {
  const e = inner;
  ...
  const baseCrumbs = (label: string) => [
    ...
    { label: p.name, onClick: () => setActive(SCOPE, { kind: 'inner', inner: { entry: 'overview' } }) },
    { label },
  ];
  ...
  if (e.entry === 'overview') {
    return (
      <ProjectOverviewView
        ...
        toast_msg={toast_msg}
        onJump={(c) => setActive(SCOPE, c)}
      />
    );
  }

  if (e.entry === 'settings' || e.entry === 'local') {
    const file = e.entry === 'settings' ? p.settings : p.local_settings;
    const filePath = file?.source_path
      || (e.entry === 'local' ? `${p.path}/.claude/settings.local.json` : `${p.path}/.claude/settings.json`);
    const initial = file?.raw ? JSON.stringify(file.raw, null, 2) : '{}';
    const title = e.entry === 'local' ? 'settings.local.json' : 'settings.json';
    const chip = e.entry === 'local'
  ...
  if (e.entry === 'memory') {
    return <ProjectMemoryScreen embedded sidebar={null} projectId={p.id} />;
  }

  if (e.entry === 'rule') {
    const r = rules.find(x => x.name === e.name);
    if (!r) return <MissingView label={e.name} onBack={() => setActive(SCOPE, { kind: 'inner', inner: { entry: 'overview' } })} />;
    return (
      <EntryDetailScreen
        embedded
        railKey="projects"
        railProjectId={p.id}
        crumbs={baseCrumbs(r.name)}
        title={r.name}
  ...
  if (e.entry === 'command') {
    ...
    if (!cmd) return <MissingView label={`/${e.name}`} onBack={() => setActive(SCOPE, { kind: 'inner', inner: { entry: 'overview' } })} />;
    return (
      <EntryDetailScreen
        embedded
        railKey="projects"
        railProjectId={p.id}
        crumbs={baseCrumbs(`/${cmd.name}`)}
        title={`/${cmd.name}`}
  ...
  if (e.entry === 'agent') {
    const a = agents.find(x => x.name === e.name);
    if (!a) return <MissingView label={e.name} onBack={() => setActive(SCOPE, { kind: 'inner', inner: { entry: 'overview' } })} />;
    return (
      <EntryDetailScreen
        embedded
        railKey="projects"
        railProjectId={p.id}
        crumbs={baseCrumbs(a.name)}
        title={a.name}
  ...
  if (e.entry === 'skill') {
    const s = skills.find(x => x.name === e.name);
    if (!s) return <MissingView label={e.name} onBack={() => setActive(SCOPE, { kind: 'inner', inner: { entry: 'overview' } })} />;
    return (
      <SkillDetailScreen
        embedded
        railKey="projects"
        railProjectId={p.id}
        crumbs={baseCrumbs(s.name)}
        title={s.name}
  ...
  // keybindings 不在项目侧；其它未匹配 → fallback 概览
  return (
    <ProjectOverviewView
      project={p}
      setNewKind={setNewKind}
...
function ProjectOverviewView({ project: p, setNewKind, onConfirmRemove, toast_msg, onJump }: {
  project: ProjectSnap;
  setNewKind: (k: EntryKind | null) => void;
  onConfirmRemove: () => void;
  toast_msg: (msg: string, tone?: 'info' | 'success' | 'error') => void;
  ...
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', background: 'var(--cc-bg)', overflow: 'hidden', minWidth: 0, minHeight: 0 }}>
      <Topbar
        crumbs={[{ label: '项目' }, { label: p.name }, { label: '概览' }]}
        right={
          <>
            <button className="cc-btn ghost" onClick={async () => {
              try { await revealInFinder(p.path); } catch (e) { toast_msg(`无法打开 Finder：${String(e)}`, 'error'); }
            }}>在 Finder 中显示</button>
            <button className="cc-btn ghost" style={{ color: 'var(--cc-muted)' }} onClick={onConfirmRemove}>移除项目</button>
          </>
        }
      />
      <div style={{ flex: 1, overflow: 'auto', padding: '26px 36px 36px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(6,1fr)', gap: 10, marginBottom: 24 }}>
          {[
            { l: 'settings', v: settingsFieldCount || '—', s: '字段', target: { entry: 'settings' as const } },
            { l: 'local', v: localFieldCount || '—', s: '覆盖', target: p.local_settings ? { entry: 'local' as const } : null },
            { l: 'memory', v: p.memory ? memorySize : '—', s: 'CLAUDE.md', target: { entry: 'memory' as const } },
            { l: '命令', v: commands.length, s: '斜杠', target: null },
            { l: 'Skills', v: skills.length, s: '技能', target: null },
            { l: 'Agents', v: agents.length, s: '子代理', target: null },
          ].map((x, i) => (
            <div
              key={i}
              onClick={() => x.target && onJump({ kind: 'inner', inner: x.target })}
              style={{
                background: 'var(--cc-bg-raised)', border: '1px solid var(--cc-line)', borderRadius: 10,
                padding: '14px 16px', cursor: x.target ? 'pointer' : 'default',
              }}
            >
        ...
        <div style={{ marginBottom: 18 }}>
          ...
          {commands.length === 0 ? (
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 10 }}>
              {commands.map((c, i) => (
                <div key={i} onClick={() => onJump({ kind: 'inner', inner: { entry: 'command', name: c.name } })} style={{ cursor: 'pointer', background: 'var(--cc-bg-raised)', border: '1px solid var(--cc-line)', borderRadius: 10, padding: '12px 14px', display: 'flex', gap: 10, alignItems: 'flex-start' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 18 }}>
          <div>
            ...
            {skills.length === 0 ? (
              ...
              <div
                key={i}
                onClick={() => onJump({ kind: 'inner', inner: { entry: 'skill', name: s.name } })}
          ...
          <div>
            ...
            {agents.length === 0 ? (
              ...
              <div
                key={i}
                onClick={() => onJump({ kind: 'inner', inner: { entry: 'agent', name: a.name } })}
                style={{ cursor: 'pointer', background: 'var(--cc-bg-raised)', border: '1px solid var(--cc-line)', borderRadius: 10, padding: '12px 14px', marginBottom: 8, display: 'flex', gap: 10, alignItems: 'center' }}
              >
